#This script evaluates ABC models on eQTL data by the Sign test
...
for(i in 1:nrow(inputList)){
  abc <- fread(inputList$probePath[i],data.table = FALSE)
...
for(i in 1:nrow(inputList)){
  ...
  positives <- as.numeric(table(abcCaviarShort$eQTLMatch)[2])
...
differences$Diff[differences$Diff > upperY] <- upperY
differences$Diff[differences$Diff < lowerY] <- lowerY
plotBox(diffTable = differences, plotMode = "Capped", capVal = opt$cap)
